...
<button type="button" class="plyr__control cd-video-btn" aria-label="Play video" data-plyr="play">
  ...
  testimonialSwiper.on('slideChange', () => {
    ...
    const nextSlide =
      testimonialSwiper.slides[testimonialSwiper.activeIndex + 1];
    const prevSlide =
      testimonialSwiper.slides[testimonialSwiper.activeIndex - 1];
    if (nextSlide) {
  ...
  const solutionRight = document.querySelectorAll(
    '[cd="home-solution-right"]'
  )!;
  ...
  const videoPopupVideo = document.getElementById(
    'hero-vid'
  ) as HTMLVideoElement;
